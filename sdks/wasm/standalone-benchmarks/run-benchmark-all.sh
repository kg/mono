...
SRC_DIR_NATIVE=$(realpath $BENCHMARK/bin/Debug/netcoreapp2.1)
SRC_DIR_WASM=$(realpath $BENCHMARK/bin/WASM/Debug/netcoreapp2.1)
...
rm -f $SRC_DIR_NATIVE/*.so
rm -f $SRC_DIR_WASM/*.so
...
mono $SRC_DIR_NATIVE/$BENCHMARK.dll 2>&1 | tee -a $LOGFILE | grep ^\>\>\>
echo \# Mono \(interpreter\) | tee -a $LOGFILE
mono --interpreter --interp=interp-only $SRC_DIR_NATIVE/$BENCHMARK.dll 2>&1 | tee -a $LOGFILE | grep ^\>\>\>
echo \# Mono \(full AOT\) | tee -a $LOGFILE
mono --aot=full $SRC_DIR_NATIVE/$BENCHMARK.dll >>$LOGFILE 2>&1
mono $SRC_DIR_NATIVE/$BENCHMARK.dll 2>&1 | tee -a $LOGFILE | grep ^\>\>\>
echo \# WebAssembly \(interpreter, node.js\) | tee -a $LOGFILE
mono --debug $PACKAGER --out=$OUT_DIR $SRC_DIR_WASM/$BENCHMARK.dll --template=$TEMPLATE_PATH >>$LOGFILE 2>&1
node test-runner.js $BENCHMARK wasm/$BENCHMARK 2>&1 | tee -a $LOGFILE | grep \>\>\>
echo \# WebAssembly \(AOT, node.js\) | tee -a $LOGFILE
mono --debug $PACKAGER --linker --aot --builddir=$AOT_OUT_DIR/obj --appdir=$AOT_OUT_DIR $SRC_DIR_WASM/$BENCHMARK.dll --mono-sdkdir=$MONO_SDK_DIR --emscripten-sdkdir=$EMSCRIPTEN_SDK_DIR --template=$TEMPLATE_PATH
